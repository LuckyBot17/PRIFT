...
class BeerConceptOccurrenceDataset(Dataset):
    """将 Beer Concept Occurrence CSV 数据封装为可用于 DataLoader 的数据集。"""
    ...
    def __init__(
        ...
        if not os.path.exists(csv_path):
            raise ValueError(f"{csv_path} 不存在，请确认 beer-concept-occurrence 数据已放置在指定目录。")
...
def build_group_mapping(
    ...
    对于 beer-concept-occurrence 数据集：
    - group_id 0: label=0 (negative), concept_absent
    - group_id 1: label=0 (negative), concept_present
    - group_id 2: label=1 (positive), concept_absent
    - group_id 3: label=1 (positive), concept_present
    """
    paths = [train_csv]
    if eval_csv:
...
def get_group_description(group_id: int) -> str:
    descriptions = {
        0: "negative-without-concept",
        1: "negative-with-concept",
        2: "positive-without-concept",
        3: "positive-with-concept",
    }
...
def compute_scale(
    ...
    encoder.eval()
    num_concepts = confounder_embeddings.size(0)
    normalized_concepts = F.normalize(confounder_embeddings, p=2, dim=1)
    scale_values: List[List[float]] = [[] for _ in range(num_concepts)]
        ...
        with torch.no_grad():
            ...
            if use_true_confounder:
                ...
                if partial_a:
                    ...
                    sensitive = inference_a(embeddings, normalized_concepts).long()

            for concept_id in range(num_concepts):
                mask = sensitive == concept_id
                if mask.any():
                    concept_embeddings = F.normalize(embeddings[mask], p=2, dim=1)
                    inner = torch.mm(concept_embeddings, normalized_concepts[concept_id].unsqueeze(1)).squeeze(1)
                    scale_values[concept_id].extend(inner.detach().cpu().tolist())

    mean_scales = [float(np.mean(values)) if values else 0.0 for values in scale_values]
    concept_names_list = list(concept_names) if concept_names else [f"concept_{idx}" for idx in range(num_concepts)]
    for idx, value in enumerate(mean_scales):
        print(f"{concept_names_list[idx]} scale: {value:.4f}")
    return torch.tensor(mean_scales, device=device)


# [MOD] 新增：用 scale 的均值对齐/居中等多种去偏策略
def debias_by_concept(
    ...
    for concept_id in range(concept_embeddings.size(0)):
        mask = (sensitive == concept_id)
        if not mask.any():
            continue
        base = adjusted[mask]
        s_vec = concept_embeddings[concept_id].unsqueeze(0)  # (1, d)
        ...
        if mode == "projection":
            ...
            proj = torch.mm(base, s_vec.t()).squeeze(1)  # (n,)
            centered = (proj - scales[concept_id]).unsqueeze(1)
            remove = strength * centered * s_vec
        elif mode == "mean":
            remove = strength * scales[concept_id] * s_vec  # broadcast
        else:
...
def collect_debiased_embeddings(
    ...
    返回:
        ...
        labels: 样本标签
        concepts: 样本真实概念存在指示（0=无概念, 1=有概念）
    """
    encoder.eval()
    ...
    for batch in tqdm(dataloader, desc=description):
        ...
        concept_real = (confounders % 2).long()

        embeddings = encode_text_batch(encoder, batch, device)

        if use_true_confounder:
            sensitive = concept_real
        else:
            if partial_a:
                sensitive = supervised_inference_a(embeddings, probe)
                sensitive = (sensitive % 2).long()
            else:
                sensitive = inference_a(embeddings, concept_embeddings).long()

        # [MOD] 使用新的去偏函数（真正用到 scales）
        adjusted = debias_by_concept(
            embeddings, sensitive, concept_embeddings, scales,
            mode=debias_mode, strength=debias_strength
        )

        all_features.append(adjusted.detach().cpu())
        all_labels.append(labels.detach().cpu())
        all_concepts.append(concept_real.detach().cpu())

    features_tensor = torch.cat(all_features, dim=0)
    labels_tensor = torch.cat(all_labels, dim=0)
    concepts_tensor = torch.cat(all_concepts, dim=0)
    return features_tensor, labels_tensor, concepts_tensor
...
class GroupBalancedBatchSampler(Sampler[List[int]]):
    def __init__(
        ...
        self.num_groups = num_labels * num_concepts

        group_ids_full = labels * num_concepts + concepts
...
def train_linear_head(
    ...
    """
    dataset = TensorDataset(features, labels, concepts)

    num_groups = num_labels * num_concepts
    group_ids_full = labels * num_concepts + concepts
    ...
    if sampler_type == "balanced":
        gb_sampler = GroupBalancedBatchSampler(labels, concepts, batch_size, num_labels, num_concepts)
    ...
    for epoch in range(epochs):
        ...
        for batch_features, batch_labels, batch_concepts in loader:
            batch_features = batch_features.to(device)
            batch_labels = batch_labels.to(device)
            batch_concepts = batch_concepts.to(device)

            optimizer.zero_grad()
            logits = classifier(batch_features)
            per_sample_loss = criterion(logits, batch_labels)

            group_ids = batch_labels * num_concepts + batch_concepts
...
def run_epoch(
    ...
    concept_embeddings = F.normalize(confounder_embeddings, p=2, dim=1).to(device)
    scales = scales.to(device)
    ...
    concept_names_list = list(concept_names) if concept_names else [f"concept_{i}" for i in range(num_concepts)]
    label_names = list(label_texts) if label_texts else [f"label_{i}" for i in range(num_labels)]

    predictions: List[int] = []
    labels_all: List[int] = []
    concepts_all: List[int] = []
    feature_concept0: List[np.ndarray] = []
    feature_concept1: List[np.ndarray] = []
    concept0_pred: List[int] = []
    concept0_gt: List[int] = []
    concept1_pred: List[int] = []
    concept1_gt: List[int] = []

    correct_combo = np.zeros((num_labels, num_concepts), dtype=float)
    total_combo = np.zeros((num_labels, num_concepts), dtype=float)
    ...
    for batch in tqdm(dataloader, desc="Evaluating"):
        with torch.no_grad():
            ...
            concept_real_cpu = concept_real.detach().cpu()
            pred_cpu = pred.detach().cpu()

            predictions.extend(pred_cpu.numpy().tolist())
            labels_all.extend(labels_cpu.numpy().tolist())
            concepts_all.extend(concept_real_cpu.numpy().tolist())

            for label_idx in range(num_labels):
                for concept_idx in range(num_concepts):
                    mask = (labels_cpu == label_idx) & (concept_real_cpu == concept_idx)
                    if mask.any():
                        matches = (pred_cpu[mask] == label_idx).sum().item()
                        total = mask.sum().item()
                        correct_combo[label_idx, concept_idx] += matches
                        total_combo[label_idx, concept_idx] += total

            for idx in range(len(pred_cpu)):
                if concept_real_cpu[idx] == 0:
                    concept0_pred.append(int(pred_cpu[idx]))
                    concept0_gt.append(int(labels_cpu[idx]))
                else:
                    concept1_pred.append(int(pred_cpu[idx]))
                    concept1_gt.append(int(labels_cpu[idx]))
    ...
    for label_idx in range(num_labels):
        ...
        for concept_idx in range(num_concepts):
            acc = safe_div(correct_combo[label_idx, concept_idx], total_combo[label_idx, concept_idx])
            print(
                f"Accuracy for label={label_name}, concept={concept_names_list[concept_idx]}: "
                f"{acc:.4f} (样本数: {int(total_combo[label_idx, concept_idx])})"
            )

    if feature_concept0:
        _ = torch.from_numpy(np.mean(np.stack(feature_concept0), axis=0))
    if feature_concept1:
        _ = torch.from_numpy(np.mean(np.stack(feature_concept1), axis=0))

    if concept0_gt and concept1_gt:
        cm_concept0 = confusion_matrix(concept0_gt, concept0_pred, labels=list(range(num_labels)))
        cm_concept1 = confusion_matrix(concept1_gt, concept1_pred, labels=list(range(num_labels)))

        dp_concept0 = safe_div(cm_concept0[:, 1].sum(), cm_concept0.sum())
        dp_concept1 = safe_div(cm_concept1[:, 1].sum(), cm_concept1.sum())

        tpr_concept0 = safe_div(cm_concept0[1, 1], cm_concept0[1].sum())
        tpr_concept1 = safe_div(cm_concept1[1, 1], cm_concept1[1].sum())

        fpr_concept0 = safe_div(cm_concept0[0, 1], cm_concept0[0].sum())
        fpr_concept1 = safe_div(cm_concept1[0, 1], cm_concept1[0].sum())

        print("\n公平性指标：")
        print(f"  Demographic parity gap: {abs(dp_concept0 - dp_concept1):.4f}")
        print(f"  Equal opportunity gap (TPR diff): {abs(tpr_concept0 - tpr_concept1):.4f}")
        print(f"  Equalized odds gap: {0.5 * (abs(fpr_concept0 - fpr_concept1) + abs(tpr_concept0 - tpr_concept1)):.4f}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TIE 在 Beer Concept Occurrence 文本数据集上的实现")
    parser.add_argument(
        "--train_csv",
        type=str,
        default=os.path.join("datasets", "beer-concept-occurrence", "train.csv"),
        help="Beer Concept Occurrence 训练集 CSV 路径",
    )
    parser.add_argument(
        "--eval_csv",
        type=str,
        default=os.path.join("datasets", "beer-concept-occurrence", "val.csv"),
        help="Beer Concept Occurrence 验证/测试集 CSV 路径",
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[
            "a beer review expressing negative sentiment",
            "a beer review expressing positive sentiment",
        ],
    ...
    parser.add_argument(
        ...
        nargs=2,
        default=[
            "a beer review that does not mention the targeted concept",
            "a beer review that emphasizes the targeted concept",
        ],
        metavar=("ABSENT_PROMPT", "PRESENT_PROMPT"),
        help="描述概念缺失/存在的提示文本（共 2 个，按概念 ID 顺序）。",
    )
    parser.add_argument(
        "--concept_names",
        nargs=2,
        default=["concept_absent", "concept_present"],
        metavar=("ABSENT_NAME", "PRESENT_NAME"),
        help="概念缺失/存在的名称，用于日志展示与公平性分析。",
    )
    parser.add_argument(
        "--text_template",
        type=str,
...
def main() -> None:
    ...
    train_dataset = BeerConceptOccurrenceDataset(
    ...
    eval_dataset = BeerConceptOccurrenceDataset(
    ...
    concept_texts = list(args.concept_prompts)
    concept_state_names = list(args.concept_names)

    confounder_embeddings = embed_sentences(
        encoder=encoder,
        tokenizer=tokenizer,
        sentences=concept_texts,
        device=device,
        max_length=args.max_length,
    )
    ...
    if args.partial_a:
        ...
        num_concept_classes = len(concept_state_names)
        probe = load_probe(args.confounder_head_path, hidden_size, num_concept_classes, device)

    concept_embeddings = F.normalize(confounder_embeddings, p=2, dim=1)

    print("Estimating concept-specific scales on training data...")
    ...
    if train_linear_head_flag:
        ...
        classifier = train_linear_head(
            ...
            concepts=train_concepts_tensor,
            device=device,
        ...
        del train_features, train_labels_tensor, train_concepts_tensor, train_logits, train_pred
    ...
    print(f"  - 混杂变量（概念状态）：{concept_state_names[0]}, {concept_state_names[1]}")
    print(f"  - 四种情况：")
    print(f"    1. label=0, concept={concept_state_names[0]} (group_id=0)")
    print(f"    2. label=0, concept={concept_state_names[1]} (group_id=1)")
    print(f"    3. label=1, concept={concept_state_names[0]} (group_id=2)")
    print(f"    4. label=1, concept={concept_state_names[1]} (group_id=3)")
    print("=" * 80)
    run_epoch(
        encoder=encoder,
